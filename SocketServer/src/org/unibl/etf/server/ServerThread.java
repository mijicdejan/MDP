		...
		try {
			...
			while(loggedIn) {
				...
				if("LOGOUT".equals(requestType)) {
					...
					ObjectOutputStream monitoringOut = Main.getMonitorOut(username);
					monitoringOut.writeObject("END");
					monitoringOut.flush();
					Main.removeMonitorIn(username);
					Main.removeMonitorOut(username);
					loggedIn = false;
					out.writeObject("END");
					out.flush();
					...
					if(groupFile.exists()) {
						messages = MessagesManager.getInstance().deserializeWithGson(groupFile);
					}
