...
public class MainForm extends JFrame {
	...
	public void initUsersPane() {
		...
		int y = 10;
		groupButton = new JButton("Svi korisnici");
		groupButton.setBorder(new LineBorder(Color.BLACK));
		groupButton.setBackground(Color.WHITE);
		groupButton.setToolTipText("Svi korisnici");
		groupButton.setBackground(Color.LIGHT_GRAY);
		pressedButton = groupButton;
		btnAllClickedAction();
		groupButton.setBounds(x, y, 160, 30);
	...
	private void btnAllClickedAction() {
		pressedButton.setBackground(Color.WHITE);
		groupButton.setBackground(Color.LIGHT_GRAY);
		pressedButton = groupButton;
		getAllGroupMessages();
	...
	public void getAllGroupMessages() {
		try {
	...
	private void btnSendMessageClicked() {
		...
		if(pressedButton.equals(groupButton)) {
			String messageText = taMessage.getText();
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern(DATE_TIME_FORMAT);
			String time = LocalDateTime.now().format(formatter);
			Message message = new Message(user, null, messageText, time);
			try {
				out.writeObject("NEW_GROUP_MESSAGE");
	...
	public void setMessages(ArrayList<Message> messages) {
		...
		for(int i = 0; i < messages.size(); i++) {
			...
			if(sender.equals(user)) {
				...
				taName.setDisabledTextColor(Color.BLACK);
				...
				taTime.setDisabledTextColor(Color.BLACK);
	...
	public User getClickedUser() {
		return buttons.get(pressedButton);
	}
